<template>
  <div class="dropdown">
    <button
      class="btn btn-secondary dropdown-border no-border no-outline py-2 px-2"
      type="button"
    ...
    <div class="dropdown-menu menu" aria-labelledby="dropdownMenuButton">
      <a
        class="dropdown-item"
        href="#"
        v-for="option in store.state.ordering_options"
        :key="option.id"
        @click="handleOrdering(option.name, option.ordering_param)"
        >{{ option.name }}</a
      >
    </div>
  </div>
