<template>
  <div class="container py-5 left-mar">
    <div class="row">
      <div
        v-for="item in store.state.products"
        class="col-12 col-sm-6 col-md-4 col-lg-4 item mb-4"
      >
        <div
          class="card hover-box-shadow h-100 no-border sizing"
          @click="$router.push(`/product/${item.id}`)"
        >
          <div class="d-flex justify-content-center no-border">
            <img
...
.rounded-full {
  ...
  border-radius: 1px;
  --border-opacity: 1;
  border-color: rgba(200, 199, 199, var(--border-opacity));
  --bg-opacity: 1;
...
.main-content:has(input#modal-toggle:checked) {
  visibility: hidden;
}

/* Show modal when open */
body:has(input#modal-toggle:checked) .modal {
  display: block;
}
